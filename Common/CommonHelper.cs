...
using DonCami.Stardew.Common.UI;
...
namespace DonCami.Stardew.Common;
...
internal static class CommonHelper
{
    ...
    /// <summary>
    ///     The width of the horizontal and vertical scroll edges (between the origin position and start of content
    ///     padding).
    /// </summary>
    public static readonly Vector2 ScrollEdgeSize = new(CommonSprites.Scroll.TopLeft.Width * Game1.pixelZoom,
    ...
    public static Vector2 GetPlayerTile(Farmer? player)
    {
        ...
        return
            new Vector2((int)(position.X / Game1.tileSize),
                (int)(position.Y /
                      Game1.tileSize)); // note: player.getTileLocationPoint() isn't reliable in many cases, e.g. right after a warp when riding a horse
    }
    ...
    public static Vector2 DrawHoverBox(SpriteBatch spriteBatch, string label, in Vector2 position, float wrapWidth)
    {
        ...
        var labelSize =
            spriteBatch.DrawTextBlock(Game1.smallFont, label, position + new Vector2(gutterSize),
                wrapWidth); // draw text to get wrapped text dimensions
        IClickableMenu.drawTextureBox(spriteBatch, Game1.menuTexture, new Rectangle(0, 256, 60, 60), (int)position.X,
            (int)position.Y, (int)labelSize.X + paddingSize + gutterSize, (int)labelSize.Y + paddingSize, Color.White);
        spriteBatch.DrawTextBlock(Game1.smallFont, label, position + new Vector2(gutterSize),
            wrapWidth); // draw again over texture box
    ...
    public static void DrawButton(SpriteBatch spriteBatch, in Vector2 position, in Vector2 contentSize,
        out Vector2 contentPos, out Rectangle bounds, int padding = 0)
    {
        DrawContentBox(
            spriteBatch,
            CommonSprites.Button.Sheet,
            CommonSprites.Button.Background,
            CommonSprites.Button.Top,
            CommonSprites.Button.Right,
            CommonSprites.Button.Bottom,
            CommonSprites.Button.Left,
            CommonSprites.Button.TopLeft,
            CommonSprites.Button.TopRight,
            CommonSprites.Button.BottomRight,
            CommonSprites.Button.BottomLeft,
            position,
            contentSize,
            out contentPos,
            out bounds,
    ...
    /// <param name="padding">The padding between the content and border.</param>
    public static void DrawScroll(SpriteBatch spriteBatch, in Vector2 position, in Vector2 contentSize,
        out Vector2 contentPos, out Rectangle bounds, int padding = 5)
    {
        DrawContentBox(
            spriteBatch,
            CommonSprites.Scroll.Sheet,
            in CommonSprites.Scroll.Background,
            CommonSprites.Scroll.Top,
            CommonSprites.Scroll.Right,
            CommonSprites.Scroll.Bottom,
            CommonSprites.Scroll.Left,
            CommonSprites.Scroll.TopLeft,
            CommonSprites.Scroll.TopRight,
            CommonSprites.Scroll.BottomRight,
            CommonSprites.Scroll.BottomLeft,
            position,
            contentSize,
            out contentPos,
            out bounds,
    ...
    /// <param name="padding">The padding between the content and border.</param>
    public static void DrawContentBox(SpriteBatch spriteBatch, Texture2D texture, in Rectangle background,
        in Rectangle top, in Rectangle right, in Rectangle bottom, in Rectangle left, in Rectangle topLeft,
        in Rectangle topRight, in Rectangle bottomRight, in Rectangle bottomLeft, in Vector2 position,
        in Vector2 contentSize, out Vector2 contentPos, out Rectangle bounds, int padding)
    {
        GetContentBoxDimensions(topLeft, contentSize, padding, out var innerWidth, out var innerHeight,
            out var outerWidth, out var outerHeight, out var cornerWidth, out var cornerHeight);
        var x = (int)position.X;
        var y = (int)position.Y;

        // draw scroll background
        spriteBatch.Draw(texture, new Rectangle(x + cornerWidth, y + cornerHeight, innerWidth, innerHeight), background,
            Color.White);

        // draw borders
        spriteBatch.Draw(texture, new Rectangle(x + cornerWidth, y, innerWidth, cornerHeight), top, Color.White);
        spriteBatch.Draw(texture,
            new Rectangle(x + cornerWidth, y + cornerHeight + innerHeight, innerWidth, cornerHeight), bottom,
            Color.White);
        spriteBatch.Draw(texture, new Rectangle(x, y + cornerHeight, cornerWidth, innerHeight), left, Color.White);
        spriteBatch.Draw(texture,
            new Rectangle(x + cornerWidth + innerWidth, y + cornerHeight, cornerWidth, innerHeight), right,
            Color.White);

        // draw corners
        spriteBatch.Draw(texture, new Rectangle(x, y, cornerWidth, cornerHeight), topLeft, Color.White);
        spriteBatch.Draw(texture, new Rectangle(x, y + cornerHeight + innerHeight, cornerWidth, cornerHeight),
            bottomLeft, Color.White);
        spriteBatch.Draw(texture, new Rectangle(x + cornerWidth + innerWidth, y, cornerWidth, cornerHeight), topRight,
            Color.White);
        spriteBatch.Draw(texture,
            new Rectangle(x + cornerWidth + innerWidth, y + cornerHeight + innerHeight, cornerWidth, cornerHeight),
    ...
    /// <param name="duration">
    ///     The number of milliseconds during which to keep the message on the screen before it fades (or
    ///     <c>null</c> for the default time).
    /// </param>
    public static void ShowInfoMessage(string message, int? duration = null)
    {
        Game1.addHUDMessage(new HUDMessage(message, HUDMessage.error_type)
            { noIcon = true, timeLeft = duration ?? HUDMessage.defaultTime });
    }
    ...
    public static void GetScrollDimensions(Vector2 contentSize, int padding, out int innerWidth, out int innerHeight,
        out int labelOuterWidth, out int outerHeight, out int borderWidth, out int borderHeight)
    {
        GetContentBoxDimensions(CommonSprites.Scroll.TopLeft, contentSize, padding, out innerWidth, out innerHeight,
            out labelOuterWidth, out outerHeight, out borderWidth, out borderHeight);
    }
    ...
    /// <param name="borderWidth">The width of the left and right border textures.</param>
    /// <param name="borderHeight">The height of the top and bottom border textures.</param>
    public static void GetContentBoxDimensions(Rectangle topLeft, Vector2 contentSize, int padding, out int innerWidth,
        out int innerHeight, out int outerWidth, out int outerHeight, out int borderWidth, out int borderHeight)
    {
    ...
    public static Vector2 DrawTextBlock(this SpriteBatch batch, SpriteFont font, string? text, in Vector2 position,
        float wrapWidth, in Color? color = null, bool bold = false, float scale = 1)
    {
        if (text == null)
        ...
        foreach (var word in words)
        {
            ...
            if (bold)
                ...
                batch.DrawString(font, word, wordPosition, color ?? Color.Black, 0, Vector2.Zero, scale,
    ...
    public static void InterceptErrors(this IMonitor monitor, string verb, Action action,
    ...
    /// <param name="action">The action to invoke.</param>
    /// <param name="onError">A callback invoked if an error is intercepted.</param>
    public static void InterceptErrors(this IMonitor monitor, string verb, string? detailedVerb, Action action,
        Action<Exception>? onError = null)
    {
        try
        {
    ...
    {
        var basePath = mod.Helper.DirectoryPath;
